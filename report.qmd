...
1. **Stage I — Data preparation.** All required spatial datasets were collected from public sources, preprocessed into a consistent format, and validated. The resulting layers are provided in the project data folder and serve as fixed inputs to all subsequent analysis.
2. **Stage II — Thematic analysis.** All analyses were conducted in R from those processed files. The workflow described in Sections 3.5–3.9 is fully reproducible from the provided data using the accompanying R scripts.
...
The municipality boundary was fetched from the PDOK WFS (Kadaster bestuurlijke gebieden), filtered to municipality code `GM0503`, and dissolved to a single polygon. Available WFS layers were auto-discovered via `GetCapabilities`; multiple bounding-box orientations were tried to handle GeoServer axis-order quirks. CBS gebiedsindelingen (years 2022–2024) was used as a fallback. Neighbourhood polygons (*buurten*) were fetched from the PDOK wijkenbuurten WFS, clipped to the municipality boundary, and filtered to buurt-level rows (`bu_code` prefix `BU`) to exclude wijk- and gemeente-level aggregates.

**Yuexiu (Guangzhou)**

The district boundary and *jiēdào* subdistrict polygons were obtained from GADM v4.1, filtered to `NAME_2 = "Guangzhou"`, and dissolved to a single city polygon. The GADM GeoPackage (~200 MB) is downloaded automatically if not present locally.

---

### Step 2 — Vector layers (green space, water, roads)

**Delft — green space and water (BGT)**

Green space and water were fetched from the BGT (Basisregistratie Grootschalige Topografie) via the PDOK OGC API Features endpoint with automatic pagination. Two feature types were collected: `groenVoorziening` (maintained green infrastructure) and `bos` (forest and woodland). Water bodies were fetched as BGT `waterdeel`, covering canals, rivers, ponds, and drainage channels. BGT was preferred over OSM because Dutch OSM data is derived from BGT via a lossy conversion; BGT carries 20 cm positional accuracy and is the legally authoritative source. OSM was used as a fallback for both layers.

**Yuexiu — green space, water, and roads (OSM)**

All vector layers were fetched from OpenStreetMap via the Overpass API. Green space was queried using the `leisure=`, `landuse=`, and `natural=` tags; water bodies using `waterway=` and `natural=water`. Requests were retried automatically with exponential backoff on HTTP 429 responses.

**Roads — both cities (OSM)**

Road networks were fetched using the `highway=` tag for both cities. A bug fix was applied: OSM ways with `highway=` that form a closed loop (roundabouts, dead-end loops) are stored as `LineString` regardless of node topology; without this fix, closed ways are parsed as filled `Polygon` geometries, corrupting network topology. A zero-distance buffer was applied to polygon geometries only to repair self-intersections, and explicitly not applied to linestrings to preserve road network topology.
...
|---|---|---|---|
| WorldPop | Both | 100 m | worldpop.org (COG range request) |
| Sentinel-2 NDVI | Both | 10 m | Google Earth Engine (cloud-free median composite) |
| ESA WorldCover | Both | 10 m | AWS S3 COG tiles |
| VIIRS VNP46A4 | Yuexiu only | ~500 m | NASA Earthdata (annual nighttime light composite) |
| Green space mask | Yuexiu only | 10 m | Derived from WorldCover + NDVI |

**Notes:**

- WorldPop was fetched via GDAL `/vsicurl/` COG range requests, retrieving only the AOI window. A full national file download was used as fallback.
- Sentinel-2 NDVI was computed per image as $(B8 - B4) / (B8 + B4)$ and reduced to a pixel-wise median. Copernicus STAC (`pystac-client`) was used as a fallback if GEE was unavailable.
- VIIRS nighttime light radiance was used as a socioeconomic proxy for Yuexiu, where direct income data are not publicly available.
- The Yuexiu green space mask was derived from WorldCover by retaining tree cover (class 10) and grassland (class 30) pixels, further filtered by NDVI > 0.3 to exclude bare or paved surfaces. Fragments < 100 m² were removed; adjacent pixels were dissolved then exploded to single-part polygons.

---

### Step 4 — Biodiversity occurrences (GBIF)

Species occurrence records were fetched from the GBIF REST API (`occurrence/search`) for each city bounding box, in pages of 300 up to a maximum of 5,000 records per city. Filters applied:

- `basisOfRecord = HUMAN_OBSERVATION`
- Valid decimal coordinates required
- `coordinateUncertaintyInMeters ≤ 1,000 m` (records with null uncertainty retained)

Retained attributes: `gbifID`, `species`, `genus`, `family`, `kingdom`, coordinates, uncertainty, `year`, `month`, `basisOfRecord`, `datasetName`. Raw JSON was cached locally to avoid repeat API calls.
...
Neighbourhood-level income data were fetched from CBS StatLine OData API, table `85163NED`, filtered to Delft buurt codes (`BU0503*`). Variables retrieved: mean disposable income per resident, mean income per earner, share of low-income households, share near the social minimum, share aged 65+, and address density. Data were joined to CBS buurt boundary polygons by neighbourhood code. Two buurten with suppressed CBS disclosure values were excluded from income correlation analysis.

**Yuexiu — VIIRS zonal statistics**

Mean, median, and standard deviation of VIIRS nighttime light radiance were computed per zone polygon from the raster downloaded in Step 3. Output was saved as `guangzhou_viirs_zonal.csv`.
...
Processing was applied per geometry type:

- **Vectors:** clipped with `geopandas.clip`; zero-distance buffer applied to polygons only; empty geometries removed.
- **Rasters:** masked with `rasterio.mask`; reprojected with `rasterio.warp.reproject`; bilinear resampling for continuous rasters (WorldPop, NDVI, VIIRS); nearest-neighbour resampling for categorical rasters (WorldCover).

All projected outputs were saved with a `_proj` suffix.

---

### Step 7 — Validation audit

A validation audit was run on all projected layers before analysis. Each layer was checked for:
...
- Key attribute completeness ≥ 80% non-null
- Cross-layer CRS consistency
