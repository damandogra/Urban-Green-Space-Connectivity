...
**Dataset incoroporated enable direct comparison between two cities**: This directly relates to how data sourced through OSM is lower in Chinese cities (such seen in the GBIF analysis on biodiversity) compared to that of Delft. This will enhance consistency and reliability of cross-city comparisons for direct standardization.
...
**Reproducibility of pipeline workflow**:
The entire analysis workflow is designed for full reproducibility and can be executed via running groupE/run_pipeline.R. The project relies on several R libraries, which can be installed or updated through the following script:
packages <- c("here", "sf", "terra", "dplyr", "ggplot2", "patchwork", "ggspatial")
install.packages(setdiff(packages, installed.packages()[,"Package"]))

Moreover, to support open science, our workflow follows the FAIR; Findable, Accessible, Interoperable, and Reusable data principles. Findability and accessibility of our pipeline is demonstarted through our public GitHub repository, with explicit open data sources to ensure resuability of data principles. 

### AI usage

Throughout the development of this assignment, we have been assisted by OpenAI’s
ChatGPT version GPT-5.5 and Google’s Gemini 3 Flash (Free tier). We have reviewed
and understood all suggestions made by those models, and take full responsibility for the
content being submitted. AI is used specifically to support our R coding.
