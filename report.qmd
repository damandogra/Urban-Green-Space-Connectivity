...
=======
In our code methodology a few limitations are identified. The MCDA weights were assigned and not derived from @fig-connectmaps, which means that the choice to assign weight accessibility at 30%, biodiversity and connectivity at 25%, and spatial justice at 20%, is a judgement made before carrying out the analysis. This, however, can differ with other research or planning priorities, as the weights assigned in this study have been based on nature-based solutions being central to the focus. Moreover, @fig-prioritytiers displays relative ranking, and does not display absolute urgency to where there needs immediate intervention to nature-based solution. It attempts to inform the least well-performing regions, rather than genuine crisis/urgency.

Finally, a limitation was on the fact that there has been no regard of peripherical conditions, and, in addition, different transportation methods. Different mobility networks bring different accessibility results, if for example tramlines in relationship to urban green space would be considered. Same goes for population density in relationship to accessible urban green space, even if its further away. If peripherical conditions would have been taken in regard, the outer neighbourhoods especially, could have a completely different score on a couple of the analysis, such as the buffer and the green space per capita. 
>>>>>>> 687674a (Re-write and add of discussion and reprocucibility and ai statement)
...
=======
Throughout the development of this assignment, we used by OpenAI’s ChatGPT version GPT-5.5 and Google’s Gemini 3 Flash (Free tier) as supporting tools. The tools were primarily used to assist with R programming, debugging, gathering information on analytical methods, and for improving clarity and grammar of the written report. All AI-generated suggestions were critically reviewed and adapted. The analytical decisions, methodological choices, data processing, interpretations, and conclusions were made solely by the authors. 
>>>>>>> 687674a (Re-write and add of discussion and reprocucibility and ai statement)
